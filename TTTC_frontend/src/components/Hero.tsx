...
const STATS_CONFIG = [
  { key: "years_of_experience" as const, label: "Years Experience" },
  { key: "happy_students"      as const, label: "Happy Students"  },
  { key: "branches"            as const, label: "Branches"        },
] as const;

const YT_ID = "wrZ17U1CQRE";

const Hero = memo(() => {
  // ✅ Reuse shared hook — no duplicate fetch
  const { stats } = useHome();
  const [isVideoPlaying, setIsVideoPlaying] = useState(false);

  const handlePlayClick = useCallback(() => setIsVideoPlaying(true), []);

  return (
    <section id="home" className="relative min-h-[90vh] bg-hero-gradient overflow-hidden">
      {/* ✅ Static blobs — no framer-motion needed for decorative elements */}
      <div className="absolute top-20 left-10 w-48 h-48 bg-primary/5 rounded-full blur-3xl pointer-events-none" />
      <div className="absolute top-40 right-20 w-64 h-64 bg-secondary/30 rounded-full blur-3xl pointer-events-none" />
      <div className="absolute bottom-40 left-20 w-40 h-40 bg-accent/10 rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="container mx-auto px-4 py-20 lg:py-4">
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          >
            ...
            >
              If it is <span className="text-gradient">identified</span>
            ...
            >
              Early intervention makes all the difference. Our expert team provides personalized therapy
            ...
            >
              <Button variant="playful" size="xl" asChild>
                <a href="/about-autism">Learn More <ArrowRight className="w-5 h-5 ml-2" /></a>
              </Button>
              ...
              <motion.div whileHover={{ scale: 1.01 }} transition={{ duration: 0.2 }}
                ...
                {isVideoPlaying ? (
                  <div className="aspect-video">
                    {/* ✅ iframe only mounted after user interaction */}
                    <iframe
                  ...
                  <>
                    {/* ✅ Thumbnail only — no iframe until play clicked */}
                    <img
                      src={`https://img.youtube.com/vi/${YT_ID}/maxresdefault.jpg`}
                      onError={(e) => { e.currentTarget.src = `https://img.youtube.com/vi/${YT_ID}/hqdefault.jpg`; }}
